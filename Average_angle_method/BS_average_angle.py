...
def implement_average_magnitude_betweem_two_frames(image1, image2):
    ...
    average = angle_matrix.mean()

    #get foreground matrix by subtraction
    foreground = angle_matrix - average
    ...
    foreground_matrix, binary_mask = get_mask(foreground, prvs)
    foreground_matrix = foreground_matrix.astype(np.uint8)
    binary_mask = binary_mask.astype(np.uint8)

    #write image
    cv2.imwrite('foreground_'+str(image1)+'.png',foreground_matrix)
    cv2.imwrite('binary_mask_'+str(image1)+'.png',binary_mask)
